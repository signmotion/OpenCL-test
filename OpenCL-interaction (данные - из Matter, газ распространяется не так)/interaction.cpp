...
const size_t NM = N * M;
// 1 метр содержит 10 ячеек (по 10 см)
...
int main(int argc, char** argv) {
    ...
    ss << " -D MAX_COUNT_PSTATE=" << MAX_COUNT_PSTATE;
    string options = " \
...
void initGLBuffers() {
    ...
    */
    ...
    status |= clSetKernelArg( kernel, 8, sizeof(cl_mem), (void*)&oclS1uOut );
    status |= clSetKernelArg( kernel, 9, sizeof(cl_mem), (void*)&oclS4uOut );
    status |= clSetKernelArg( kernel, 10, sizeof(cl_mem), (void*)&oclS4fOut );
    status |= clSetKernelArg( kernel, 11, sizeof(cl_mem), (void*)&oclW8fOut );
    /* - Обходимся.
...
void createWorld() {
    ...
    auto md = new Matter[ MAX_COUNT_MATTER * MAX_COUNT_PSTATE ];

    const size_t amMatter = sizeof(Matter) * MAX_COUNT_MATTER * MAX_COUNT_PSTATE;
    cout << "Память для Matter: " << amMatter / 1024 << " КБ" << endl;
    ...
    for (size_t id = 0; id < MAX_COUNT_MATTER; id++) {
        for (size_t state = 0; state < MAX_COUNT_PSTATE; state++) {
            size_t i = id * MAX_COUNT_PSTATE + state;
            md[i].molarMass = 0.0f;
            md[i].color = 0x00000000;
            md[i].tt = 0.0f;
            md[i].density = 0.0f;
        }
    }
    ...
    // Характеристики, постоянные для всех состояний
    float molarMass = 29.0f * 1000.0f;
    ...
    cout << "Память для DataS1U: " << sizeof(DataS1U) * NM / 1024 / 1024 << " МБ" << endl;
    cout << "Память для DataS4U: " << sizeof(DataS4U) * NM / 1024 / 1024 << " МБ" << endl;
    cout << "Память для DataS4F: " << sizeof(DataS4F) * NM / 1024 / 1024 << " МБ" << endl;
    cout << "Память для DataW8F: " << sizeof(DataW8F) * NM / 1024 / 1024 << " МБ" << endl;

    // На все ячейки системы всегда действует сила тяжести
    const float DEFAULT_FORCE[] = { 0.0f, -9.81f };

    // Инициализируем весь мир пустотой и силами.
    // Определяем все элементы структуры, чтобы быть уверенными в их значениях.
    id = 0;
    size_t iMatter = id * MAX_COUNT_PSTATE + VACUUM;
    ...
    iMatter = id * MAX_COUNT_PSTATE + GAS;
    for (size_t z = M / 4; z < M / 4 + AIR_BLOCK_HEIGHT; z++) {
        for (size_t x = N / 4; x < N / 4 + AIR_BLOCK_WIDTH; x++) {
            ...
            // D - плотность
            s4f[i].pressure = md[ iMatter ].density / molarMass * R * (-C0) / 3;
    ...
    status |= clSetKernelArg( kernel, 1, sizeof(cl_mem), (void*)&oclMD );
    status |= clSetKernelArg( kernel, 2, sizeof(cl_mem), (void*)&oclS1u );
    status |= clSetKernelArg( kernel, 3, sizeof(cl_mem), (void*)&oclS4u );
    status |= clSetKernelArg( kernel, 4, sizeof(cl_mem), (void*)&oclS4f );
    status |= clSetKernelArg( kernel, 5, sizeof(cl_mem), (void*)&oclW8f );
    assert(status == CL_SUCCESS);
...
void createImageMatter() {
    ...
    cl_int status = clSetKernelArg( kernel, 6, sizeof(cl_float), &dt0 );
    const cl_uint r0 = 0;
    status |= clSetKernelArg( kernel, 7, sizeof(cl_float), &r0 );
    assert(status == CL_SUCCESS);
...
void interactionGas(float dt) {
    ...
    cl_int status = clSetKernelArg( kernel, 6, sizeof(cl_float), &dt );
    const cl_uint r = rand();
    status |= clSetKernelArg( kernel, 7, sizeof(cl_uint), &r );
    assert(status == CL_SUCCESS);
